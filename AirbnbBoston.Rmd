...
#View(airbnb16)
```
#plot line graph about the monthly average 
#price in a whole year
```{r,message=FALSE}
#monthly average price
mon_avg<-NULL
mon_avg1 = airbnb16%>%group_by(date)%>%summarise(price=mean(price, na.rm=T))
#Monthly average price distribution
##plot(1:12,mon_avg,xaxt="n",main="Monthly average price of Boston on Airbnb",xlab="Month",ylab="Average price")
##axis(1,labels=Mon,at=1:12,las=1) 
#mon_avg$date1<-factor(mon_avg$date,levels = c("Jan-16","Feb-16","Mar-16","Apr-16","May-16","Jun-16","Jul-16","Aug-16","Sep-16","Oct-16","Nov-16","Dec-16"))
mon_avg1[1:12,1]=c("Apr","Aug","Dec","Feb","Jan","Jul","Jun","Mar","May","Nov","Oct","Sep")
mon_avg=mon_avg1
#a=mon_avg[c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"),]
...
```
#plot histogram of the monthly average price 
#in busy month(Oct), leisure month(Feb) and Regular month(Jun), respectively
```{r,warning=FALSE}
...
d + geom_histogram(bins = 20)+
  ...
  ylim(0,400)
#to have all three months shown on a single plot
